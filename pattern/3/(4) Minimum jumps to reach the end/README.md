...
### Basic Solution
We will start with the '0’th index and try all options. So, if the value at the current index is ‘p’, we will
try every jump in the range (1 to ‘p’) from that index. After taking a jump, we recursively try all option from that index.
